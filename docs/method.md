...
x = u F^{⊗n} as an in-place butterfly: at stage s (step 2^s),
v[i+j] ^= v[i+j+step]. Arikan writes the generator as B_N F^{⊗n} with an
...
check on this: `test_basic.py` asserts encode(encode(u)) == u, since the
...
Z- = 2Z - Z^2, Z+ = Z^2 from Z = eps and freezes the N-K worst channels.
...
each synthetic channel from m0 = 2/sigma^2, with upper update
...
inverse. The design SNR uses the same Eb/N0 calibration as the channel
...
positions with BEC at eps 0.5). The sets diverge as design SNR moves. If
...
segment go through f(a,b) = sign(a) sign(b) min(|a|,|b|), the min-sum check
...
complexity is O(L n^2) because paths hold full decision vectors and LLRs
are recomputed per bit; the paper's O(L n log n) comes from the lazy-copy
