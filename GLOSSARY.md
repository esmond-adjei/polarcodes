...
| Eb/N0 | Energy per info bit over noise density, in dB. X-axis of every curve. Differs from Es/N0 by 10log10(R). | `snr_db` args; `snr_to_sigma` converts via sigma^2 = 1/(2 R Eb/N0) |
...
| Design SNR | Eb/N0 the frozen set is optimized for. Match it to the operating point. | `design_snr_db` in `awgn_construction` |
...
| Synthetic (bit-) channel | One ordered position u_0..u_{N-1}, each seeing a different effective channel. Position 0 is worst, N-1 best. | Decided in order by `sc_decode` |
...
| Phi function | phi(x) = E[tanh(L/2)], maps mean LLR into [0,1] so the check-node update runs in the probability domain. | `_phi` / `_phi_inv` in `core.py` |
| Generator matrix (F^{⊗n}) | The polar transform, Kronecker power of [[1,0],[1,1]]. | `kron_power` (reference); `encode` (butterfly) |
| Bit-reversal (B_N) | Permutation in Arikan's G_N = B_N F^{⊗n}. Absorbed into ordering here. | `bit_reverse` documents it; outside the data path |
...
| f node | Upper-branch update: LLR of the XOR of two bits, min-sum form sign(a)sign(b) min(\|a\|,\|b\|). | `f` in `channel_sc.py` |
...
| Monte Carlo point | One (decoder, Eb/N0) measurement over n_blocks seeded messages. Needs ~10+ observed errors to mean anything. | `run_point(N, K, frozen, snr_db, ...)` |
